...
The same release also introduced `SocketsHttpHandler` with `PooledConnectionLifetime`, which does the same job at a finer grain. Each pooled connection has its own age, and they cycle individually without disposing the handler or losing its TLS session tickets. For DNS refresh specifically, that's better in basically every measurable way than rotating the whole handler.

The 2 minute default never went away, partly for back-compat and partly because not every handler is `SocketsHttpHandler`. The legacy `HttpClientHandler` doesn't have `PooledConnectionLifetime`, and people still use it for various reasons (custom handler chains, older runtimes, integration with non-managed transports). So the factory can't assume the modern primitive is available.

If you're on .NET Core 2.1 or later and using `SocketsHttpHandler` (which is the default since then), the modern recommendation is to set `SetHandlerLifetime(Timeout.InfiniteTimeSpan)` to switch off the old hammer, and let `PooledConnectionLifetime` do the rotation instead. Microsoft says this directly in [the current HttpClient guidelines](https://learn.microsoft.com/en-us/dotnet/fundamentals/networking/http/httpclient-guidelines).
